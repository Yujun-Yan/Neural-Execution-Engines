...
def run_add(current_dir, reload_from_dir_1, reload_dir_1, checkpoint_path_1):
    binary_size = 8
    d_model = 24
    ...
    with open("{}parameters.txt".format(current_dir), 'w') as fi:
            ...
            binary_size, d_model, Train_size, BATCH_SIZE, Val_size, num_layers, dff, target_vocab_size, dropout_rate, make_sym, EPOCHS_1, seq_len_1, seq_len_2, out_num_1, out_pos_1, USE_positioning_1, num_max_1, pos) + reload_from_dir_1 * "\nreload_dir_1: {}".format(reload_dir_1))
        
    Train_dataset_1, Val_dataset_1, Texmp_size = add_data(current_dir, reload_from_dir_1, reload_dir_1, num_max_1, Train_size, Val_size, end_token, binary_size)
